...
This component was converted to the locked-dependency model in **v0.1.21**. The supply-chain posture below applies to v0.1.21 and every release after it. Earlier releases predate the conversion and do not carry these guarantees; per §1, the remediation for any exposure on an earlier release is to upgrade to the current release.
...
```
https://github.com/OpenSecOps-Org/Foundation-security-services-setup/releases/download/<VERSION>/Foundation-security-services-setup-<VERSION>-sbom.cdx.json
```

Replace `<VERSION>` with the release tag (e.g. `v0.1.21`). The asset is attached to every release on the public OpenSecOps-Org remote.
...
Together they are the authoritative cross-component references; the per-component sections above describe how the project-wide posture applies to **Foundation-security-services-setup** specifically.
